$(document).ready(function() {
  ...
  function gethikinginfo() {
    ...
    $.get("/hiking", searchinfo, function(response) {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        var newDiv = $("<div>").addClass("card card-trail mt-4");
        var newDiv = $("<div class='hikingFloat'>").addClass(
          "card card-trail mt-3 mb-3"
        );
  ...
  function getweatherinfo() {
    ...
    $.get("/weather", searchinfo, function(response) {
      ...
      // created a variable to display images for response[0].current.skycode
      var rightNowImg =
        `http://blob.weather.microsoft.com/static/weather4/en-us/law/` +
        response[0].current.skycode +
        ".gif";
      console.log("skycode is " + response[0].current.skycode);
      // variable for the header (current city temp)
      var weatherHeader = $(
        "<div class = 'card' id = 'weatherCard'><div class = 'card-body dayBSBody'><p class = 'weatherHeader'>5-Day Weather Forecast" +
          "<p class = 'smallTxt dayBSBody'>The current temp for " +
          city +
          " is " +
          response[0].current.temperature +
          "\u00B0F" +
          "<br><img src = " +
          rightNowImg +
          " class = 'forecastFont'>"
      );
      // append top top of forecastDiv
      forecastDiv.append(weatherHeader);
      ...
      for (var i = 0; i < response[0].forecast.length; i++) {
        ...
        var skytxtDiv = $("<p>" + "<img src = " + conditionsImgs + " class = 'forecastFont'>" + "  " + currentDay.skytextday + "</p>");
  ...
  $(document).on("click", ".trailImg", function() {
    ...
    var trailiframe = $(
      `<iframe style="width:100%; max-width:1200px; height:410px;" frameborder="0" scrolling="no" src="https://www.hikingproject.com/widget?v=3&map=1&type=trail&id=${trailid}"></iframe>`
    );
    var name = $(this)
      .closest("div")
      .attr("data-actName");
    ...
    var Descent = $(this)
      .closest("div")
      .attr("data-descent");
    ascentp.append(`<p>Ascent:${Ascent},      Descent:${Descent}</p>`);
    var ratingp = $("<p>");
    var Ratings = $(this)
      .closest("div")
      .attr("data-rating");
